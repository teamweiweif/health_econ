...
python script/207_build_mwi2004_sdg382_spl_bridge_verification_gate.py
...
- The priority LSMS/ISA public resource-route probe is `result/priority_lsms_isa_resource_download_route_probe_summary.csv`; it checks common file-id routes without saving raw payloads.
- The priority LSMS/ISA download acceptance matrix is `result/priority_lsms_isa_download_acceptance_matrix_summary.csv`; it lists expected files and requirement rows to accept after packages are placed.
- The priority LSMS/ISA local target readmes manifest is `result/priority_lsms_isa_local_target_readme_summary.csv`; it records per-target local download acceptance README files.
...
- The priority LSMS/ISA local raw presence audit is `result/priority_lsms_isa_local_raw_presence_summary.csv`; it verifies that only Malawi 2004 has registry raw evidence, the 10 minimum-batch rows still lack raw files, and Albania raw files remain diagnostic-only.
- The priority LSMS/ISA acquisition-to-promotion handoff is `result/priority_lsms_isa_acquisition_to_promotion_handoff_summary.csv`; it maps all 19 registry waves from raw acquisition into verification, climate-linkage, promotion-packet, and registry-refresh gates without opening data writes or modeling.
- The priority LSMS/ISA dataset scope lock is `result/priority_lsms_isa_dataset_scope_lock_summary.csv`; it fixes the target as 6 countries and 11 waves from 2004-2005 through 2021-2022, with 1 current promoted anchor and 10 official raw packages still required.
- The priority LSMS/ISA acquisition route decision is `result/priority_lsms_isa_acquisition_route_decision_summary.csv`; it consolidates public route probes, access gates, session-material readiness, browser/manual starter commands, and local raw counts for the 10 download-required waves.
- The priority LSMS/ISA scoped incoming package router is `result/priority_lsms_isa_scoped_incoming_package_router_summary.csv`; it maps files placed under `temp/raw_downloads/_incoming/` only to the 10 locked download-required waves and stays fail-closed until packages are reviewed and copied.
- The priority LSMS/ISA Web GPT download control manifest is `result/priority_lsms_isa_webgpt_download_control_summary.csv`; it gives one direct-read acquisition board plus a 323-row expected core-file checklist for the 10 download-required waves.
- The priority LSMS/ISA manual download launchpad is `report/priority_lsms_isa_manual_download_launchpad.html` plus `result/priority_lsms_isa_manual_download_launchpad_summary.csv`; it gives clickable official links and target folders for the 10 browser/manual World Bank downloads.
- The priority LSMS/ISA post-download receipt handoff is `result/priority_lsms_isa_post_download_receipt_handoff_summary.csv`; it exports the 10 post-download receipt gates and 70 requirement-level gates that must pass before promotion can resume.
- The priority LSMS/ISA package-level download manifest is `result/priority_lsms_isa_package_level_download_manifest_summary.csv`; it compresses the 10 World Bank targets into package-level receipt actions while preserving the 838 expected file rows, 323 requirement-linked core rows, and 213 unique core-file names.
- The priority LSMS/ISA acquisition gap receipt board is `result/priority_lsms_isa_acquisition_gap_receipt_summary.csv`; it reconciles the 10 remaining package downloads with the 6-country / 10-wave threshold gap and row-specific receipt validation commands.
- The priority LSMS/ISA external local raw candidate audit is `result/priority_lsms_isa_external_local_raw_candidate_summary.csv`; it finds local cross-project raw-folder candidates for 7 selected rows and 3 backup rows, but keeps provenance unaccepted until official package review and receipt validation pass.
- The priority LSMS/ISA external local raw intake decision is `result/priority_lsms_isa_external_local_raw_intake_summary.csv`; it turns those local candidates into a copy-review queue with 4 selected waves ready for official-provenance review, 3 selected partial-review waves, and 3 backup waves, while keeping automatic copy and accepted provenance at zero.
- The priority LSMS/ISA external local raw staging log is `result/priority_lsms_isa_external_local_raw_staging_summary.csv`; this machine staged 3 review-ready folders into `temp/raw_downloads/` for receipt validation, copying 304 raw/documentation files while keeping accepted provenance, clean data writes, and modeling at zero/blocked.
...
- The Malawi 2004 SDG 3.8.2 SPL bridge verifier is `result/mwi2004_sdg382_spl_bridge_verification_gate_summary.csv`; it revalidates World Bank PPP/CPI source values but keeps SDG 3.8.2 blocked because the survey real-currency base is national February/March 2004 while the candidate bridge uses annual CPI 2004.
