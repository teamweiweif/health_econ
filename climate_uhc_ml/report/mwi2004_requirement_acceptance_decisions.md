...
| mechanical_raw_checks_pass_or_partial | 8 | Requirements with raw mechanical evidence that passes but still needs policy/documentation acceptance. |
| hard_blocked_requirements | 0 | Requirements still blocked even before final climate/data promotion. |
| final_verified_requirements | 8 | Requirements accepted as final raw-value verified for their stated scope. |
...
| --- | --- | --- | --- | --- | --- |
| household_person_keys | raw_value_verified_access_person_key_exclusion_policy_accepted | raw_value_verified_for_access_person_universe_with_documented_exclusions | household/exp/pov case_id joins pass; health->roster unmatched=6; roster->health unmatched=2; exception_status=policy_pending_exception_unresolved; access_resolution_status=access_person_keys_and_cost_barrier_forgone_care_verified_with_documented_exclusions; nonroster_d04_yes=0; nonroster_no_money=0. | Accepted for the stated access person-universe scope only; raw IDs are not exported and no access outcomes are imputed for roster-only health absences. | Keep the documented exclusion/no-imputation policy attached to any Malawi 2004 access or double-failure use. |
...
| health_need_and_access | raw_value_verified_access_person_key_policy_accepted | raw_value_verified_for_cost_barrier_forgone_care | d04 illness yes rows=14143; d07 no-money label hits=660; label_decision_rows=127; label_no_money_rows=660; accepted_denominator=14143; accepted_no_money_rows=631; formal_core=5130; formal_extended=5593; skip_leakage_rows=6; policy_skip_exceptions=6; skip_exception_no_money=0; d07a_leak_overlap_with_nonroster=0; explained_by_nonroster=0; access_resolution_status=access_person_keys_and_cost_barrier_forgone_care_verified_with_documented_exclusions. | Cost-barrier forgone care is accepted only for roster-matched d04==Yes rows with documented exclusions; distance/supply barriers and full nonfinancial access taxonomy remain out of scope. | Use the accepted access/person-key policy for double-failure readiness, but do not write promoted data until missing/units/recall and climate-route gates pass. |
...
| missing_codes_units_recall_skip_patterns | raw_value_verified_missing_units_recall_skip_policy_accepted | raw_value_verified_for_missing_units_recall_skip_patterns | Raw value labels and profiles exist; missing_units_recall_skip_policy_status=raw_missing_units_recall_skip_policy_verified_for_accepted_constructs; financial_units_verified=1; access_units_recall_skip_verified=1; timing_units_verified=1; geography_units_verified=1. | Accepted for CHE10/CHE25, cost-barrier access, timing, and admin/EA geography scope; SDG 3.8.2 and CHIRPS/ERA5 route remain separate blockers. | Proceed to CHIRPS/ERA5 route and promoted synthesis review; do not write data while climate route is blocked. |
...
The current decision remains fail-closed: `final_verified_requirements=8`
