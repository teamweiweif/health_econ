...
|---|---:|---|
| bundle_rows | 208 | Rows in result/direct_read_audit_bundle.csv. |
| manifest_rows | 926 | Curated artifact rows in result/direct_read_artifact_manifest.csv. |
| manifest_present_nonempty | 922 | Curated artifacts present and non-empty. |
...
| priority_lsms_isa_received_raw_schema_file_rows | 611 | Received official raw archive files included in the metadata-only schema audit. |
| priority_lsms_isa_received_raw_schema_readable_file_rows | 559 | Received official raw files readable by the schema audit. |
| priority_lsms_isa_received_raw_schema_variable_rows | 17061 | Variables extracted from the received official raw archive schema. |
| priority_lsms_isa_received_raw_schema_candidate_present_rows | 434 | Requirement candidate variables present in the received official raw archive. |
| priority_lsms_isa_received_raw_value_profile_variable_rows | 434 | Candidate requirement variables with received raw value-profile evidence. |
| priority_lsms_isa_received_raw_value_profile_nonmissing_variable_rows | 433 | Candidate variables with at least one nonmissing raw value. |
| priority_lsms_isa_received_raw_key_design_geography_profile_rows | 554 | Utility key, design, and geography variables profiled from received raw files. |
| priority_lsms_isa_received_raw_value_profile_requirements_with_profiles | 35 | Requirements with value-profile evidence available for manual review. |
| priority_lsms_isa_received_raw_semantics_variable_rows | 988 | Variable-level semantics review rows from value and utility profiles. |
...
| priority_lsms_isa_received_raw_semantics_requirement_rows | 40 | Requirement-level semantics review rows. |
| priority_lsms_isa_received_raw_semantics_missing_codes_units_recall_skip_requirement_rows | 5 | Documentation-semantics gate rows backed by review evidence. |
| priority_lsms_isa_focused_raw_value_decision_dataset_rows | 5 | Received raw datasets with focused raw value decision rows. |
| priority_lsms_isa_focused_raw_value_documentation_file_rows | 7 | Local package PDF documentation files parsed for focused raw value review. |
| priority_lsms_isa_focused_raw_value_variable_decision_rows | 810 | Variable-level focused raw value decision rows. |
| priority_lsms_isa_focused_raw_value_requirement_decision_rows | 35 | Requirement-level focused raw value decision rows. |
...
| priority_lsms_isa_household_join_dataset_rows | 5 | Received LSMS/ISA country-waves included in household join readiness audit. |
| priority_lsms_isa_household_join_file_audit_rows | 23 | Core raw files audited for household keys and required fields. |
| priority_lsms_isa_household_join_pair_audit_rows | 23 | Base-to-component join coverage checks. |
| priority_lsms_isa_household_join_complete_path_rows | 5 | Country-waves with all required household join paths ready for manual value/climate review. |
...
| mwi2004_requirement_acceptance_mechanical_pass_or_partial | 8 | Requirements with mechanical raw evidence that passes but remains short of final verification. |
| mwi2004_requirement_acceptance_final_verified | 8 | Requirements accepted as final verified for their stated scope; this does not by itself open data writes. |
...
| mwi2004_sdg382_parameter_internal_inputs_complete | 1 | Whether Malawi 2004 has raw OOP, consumption, household size, and weights needed before parameterizing current SDG 3.8.2. |
| mwi2004_sdg382_parameter_external_ppp_cpi_verified | 0 | Whether 2017 PPP and CPI/deflator parameters have been frozen for Malawi 2004 SDG 3.8.2. |
| mwi2004_sdg382_parameter_sdg382_ready | 0 | Current Malawi 2004 SDG 3.8.2 readiness under the 2026 discretionary-budget metadata. |
| mwi2004_sdg382_external_parameter_ppp_private_2017 | 249.104888916 | World Bank private-consumption PPP candidate captured for Malawi 2017. |
| mwi2004_sdg382_external_parameter_cpi_ratio_candidate | 6.11673829608 | Annual CPI candidate bridge from 2004 to 2017; not final. |
| mwi2004_sdg382_external_parameter_bridge_accepted | 0 | Whether the external PPP/CPI bridge is accepted for final SDG 3.8.2 classification. |
| mwi2004_sdg382_candidate_precheck_nonpositive_discretionary_rows | 9073 | Rows with nonpositive candidate discretionary budget under Malawi 2004 candidate SPL. |
| mwi2004_sdg382_candidate_precheck_strict_candidate_rows | 119 | Candidate SDG 3.8.2 household rows under the strict positive-discretionary-budget diagnostic variant. |
| mwi2004_sdg382_candidate_precheck_floor_candidate_rows | 122 | Candidate SDG 3.8.2 household rows under the denominator-floor sensitivity variant. |
| mwi2004_sdg382_candidate_precheck_written_to_data | 0 | Whether household-level candidate SDG 3.8.2 classifications were written to data; should remain zero. |
| mwi2004_sdg382_official_denominator_rule_accepted | 1 | Whether the official nonpositive-discretionary-budget rule is accepted from UNSD metadata. |
...
| mwi2004_health_access_policy_status | candidate_policy_ready_active_skip_and_provider_blockers | Candidate health/access construction policy status. |
| mwi2004_health_access_policy_denominator_rows | 14143 | Roster-matched d04==Yes rows under the candidate access denominator. |
| mwi2004_health_access_policy_no_money_rows | 631 | Candidate no-money forgone-care rows counted once per person row. |
| mwi2004_health_access_resolution_status | access_person_keys_and_cost_barrier_forgone_care_verified_with_documented_exclusions | Malawi 2004 access/person-key resolution status. |
| mwi2004_health_access_resolution_denominator_rows | 14143 | Accepted roster-matched acute-need denominator rows. |
| mwi2004_health_access_resolution_no_money_rows | 631 | Accepted cost-barrier forgone-care rows. |
| mwi2004_health_access_resolution_skip_no_money_rows | 0 | No-money rows among documented skip exceptions; must be zero for accepted exclusion. |
...
| priority_lsms_isa_receipt_checklist_package_received_rows | 10 | Refocused LSMS/ISA waves with original package receipt evidence. |
...
| priority_lsms_isa_official_file_receipt_expected_file_matched_rows | 814 | Expected official DDI files matched locally. |
| priority_lsms_isa_official_file_receipt_core_file_missing_rows | 349 | Core expected files still missing locally. |
...
| priority_lsms_isa_minimum_batch_missing_core_file_rows | 242 | Core official files still missing locally for the 11-wave minimum batch. |
| priority_lsms_isa_minimum_endpoint_credentialed_download_required_rows | 9 | Minimum-batch waves still requiring credentialed World Bank download after endpoint refresh. |
...
| priority_lsms_isa_next_raw_package_actions | 18 | Country-waves in the next raw package acquisition queue. |
| priority_lsms_isa_next_raw_package_minimum_remaining | 10 | Remaining minimum-batch waves requiring complete official raw packages. |
...
| priority_lsms_isa_threshold_gap_current_country_gap | 5 | Additional value-verified countries needed from the current promoted registry. |
| priority_lsms_isa_threshold_gap_current_wave_gap | 9 | Additional value-verified country-waves needed from the current promoted registry. |
| priority_lsms_isa_threshold_gap_minimum_remaining_downloads | 10 | Minimum-batch raw package downloads still required. |
| priority_lsms_isa_threshold_gap_countries_if_minimum_passes | 6 | Countries covered if all remaining minimum-batch downloads later pass verification. |
| priority_lsms_isa_threshold_gap_waves_if_minimum_passes | 11 | Country-waves covered if all remaining minimum-batch downloads later pass verification. |
| priority_lsms_isa_threshold_gap_country_buffer | 0 | Country-count buffer above the 6-country threshold after the minimum batch passes. |
| priority_lsms_isa_threshold_gap_wave_buffer | 1 | Country-wave buffer above the 10-wave threshold after the minimum batch passes. |
| priority_lsms_isa_manual_download_packet_rows | 10 | Remaining minimum-batch waves with per-wave manual download packets. |
| priority_lsms_isa_manual_download_packet_core_rows | 323 | Core-file rows listed across manual download packets. |
| priority_lsms_isa_manual_download_packet_reports | 10 | Per-IDNO markdown packet reports written. |
...
| priority_lsms_isa_unlock_board_rows | 10 | Minimum-batch promotion unlock board rows. |
| priority_lsms_isa_unlock_board_public_docs_complete | 10 | Minimum-batch rows with complete public documentation receipt. |
...
| priority_lsms_isa_browser_download_starter_rows | 10 | Minimum-batch rows with browser/manual download starter commands. |
...
| priority_lsms_isa_dataset_scope_lock_rows | 11 | Locked target country-wave rows for the dataset-promotion campaign. |
| priority_lsms_isa_dataset_scope_lock_country_rows | 6 | Countries represented if all locked target rows pass promotion gates. |
| priority_lsms_isa_dataset_scope_lock_download_required_rows | 10 | Locked target rows still requiring official raw package acquisition. |
...
| priority_lsms_isa_scoped_incoming_router_targets | 10 | Locked download-required waves covered by the scoped incoming router. |
| priority_lsms_isa_scoped_incoming_router_incoming_files | 0 | Files currently staged under temp/raw_downloads/_incoming for the scoped router. |
| priority_lsms_isa_scoped_incoming_router_pending_drop_rows | 10 | Scoped target waves still waiting for an incoming package drop. |
| priority_lsms_isa_scoped_incoming_router_copy_candidates | 0 | Scoped target waves with a suggested incoming file to copy after review. |
| priority_lsms_isa_webgpt_download_control_rows | 10 | Download-required waves in the direct-read Web GPT control manifest. |
| priority_lsms_isa_webgpt_download_expected_full_files | 838 | Expected official file rows in the Web GPT download control manifest. |
| priority_lsms_isa_webgpt_download_expected_core_files | 323 | Expected core-file rows exported for direct review. |
| priority_lsms_isa_webgpt_download_browser_manual_rows | 10 | Web GPT control rows still requiring browser/manual World Bank terms acceptance. |
...
| priority_lsms_isa_package_manifest_rows | 10 | Package-level download actions for locked World Bank targets. |
| priority_lsms_isa_package_manifest_expected_full_files | 838 | Expected official file rows preserved from the package-level manifest. |
| priority_lsms_isa_package_manifest_expected_core_rows | 323 | Requirement-linked core file rows preserved from the package-level manifest. |
| priority_lsms_isa_package_manifest_unique_core_files | 213 | Unique expected core files in the package-level manifest. |
| priority_lsms_isa_package_manifest_blocked_no_package | 10 | Package rows still lacking a local official package or incoming file. |
| priority_lsms_isa_acquisition_gap_board_rows | 10 | Rows in the unified acquisition gap and receipt board. |
| priority_lsms_isa_acquisition_gap_remaining_financial_country_gap | 5 | Financial-protection-ready countries still needed before modeling can resume. |
| priority_lsms_isa_acquisition_gap_remaining_double_failure_wave_gap | 9 | Double-failure-ready country-waves still needed before modeling can resume. |
| priority_lsms_isa_acquisition_gap_missing_package_rows | 10 | Download-required rows still lacking a local package or incoming file. |
| priority_lsms_isa_acquisition_gap_first_canary | ETH_2021_ESPS-W5_v02_M | First package to download and validate before scaling the remaining batch. |
| priority_lsms_isa_external_candidate_locked_target_with_matches | 7 | Selected rows with external local expected-file matches. |
...
| priority_lsms_isa_external_staging_executed_dataset_rows | 6 | External local raw-folder candidates copied into temp/raw_downloads for receipt validation. |
| priority_lsms_isa_external_staging_copied_file_rows | 667 | Raw/documentation files staged from external local folders. |
| priority_lsms_isa_external_staging_provenance_accepted | 0 | Staged external local packages accepted as official provenance; should remain zero until review passes. |
| priority_lsms_isa_country_wave_packet_rows | 19 | Refocused LSMS/ISA country-wave promotion packets built. |
| priority_lsms_isa_country_wave_packet_failed_gates | 288 | Refocused LSMS/ISA packet gates still blocking promotion. |
...
| priority_manual_verification_requirement_rows | 104 | Requirement rows evaluated by the manual verification decision gate. |
| priority_manual_verification_concept_rows | 169 | Concept rows evaluated by the manual verification decision gate. |
| priority_manual_verification_variable_rows | 1214 | Variable rows evaluated by the manual verification decision gate. |
| priority_manual_verification_financial_ready_countries | 0 | Countries passing financial-protection manual verification. |
| priority_manual_verification_double_failure_ready_waves | 0 | Country-waves passing double-failure manual verification. |
| priority_manual_verification_analysis_ready_candidates | 0 | Country-waves ready for harmonization-recipe review after manual verification. |
| priority_raw_receipt_original_file_rows | 0 | Original package/documentation files counted in priority receipt folders, excluding generated handoffs. |
| priority_raw_receipt_priority_targets_missing | 156 | Priority target modules still missing from original package/archive receipt coverage. |
| priority_raw_receipt_missing_package_rows | 13 | Priority datasets with no original raw package/tabular receipt yet. |
| priority_raw_receipt_complete_package_candidates | 0 | Priority datasets with original package receipt candidates ready for downstream schema/manual audits. |
| priority_official_download_full_file_rows | 965 | Full official metadata file rows available for credentialed package acquisition checks. |
| priority_official_download_documentation_links | 100 | Official metadata/documentation/access workflow links in the download dossier. |
| priority_official_download_no_original_package_rows | 13 | Download dossiers still blocked because no original raw package is present. |
| priority_public_documentation_dataset_rows | 13 | Priority datasets with public documentation receipt rows. |
...
| priority_credentialed_acquisition_dataset_rows | 13 | Priority datasets with credentialed raw-acquisition instructions. |
| priority_credentialed_acquisition_full_file_rows | 965 | Official metadata file rows in the credentialed acquisition manifest. |
| priority_credentialed_acquisition_core_file_rows | 156 | Priority core file rows to confirm after official raw download. |
| priority_credentialed_acquisition_targets_missing_before_download | 156 | Priority target modules still missing before credentialed download. |
...
| priority_endpoint_matrix_raw_download_candidate_rows | 0 | Raw download candidate endpoints detected by the official endpoint matrix. |
| priority_endpoint_matrix_credentialed_download_required_rows | 13 | Datasets still requiring credentialed raw download. |
...
| priority_download_execution_packet_rows | 13 | Manual credentialed download execution packet rows. |
| priority_download_execution_core_file_rows | 156 | Core file acceptance rows carried into the download execution packet. |
| priority_download_execution_first_pass_variable_rows | 465 | First-pass selected variable rows carried into the download execution packet. |
| priority_download_execution_raw_package_received_rows | 0 | Download execution packet datasets with original raw package receipt. |
| priority_lsms_alignment_current_campaign_rows | 13 | Current priority/backup campaign rows audited for LSMS/ISA family alignment. |
...
| priority_lsms_raw_intake_original_file_rows | 972 | Non-generated original package/documentation candidates found in target folders. |
| priority_lsms_raw_intake_missing_package_rows | 9 | Refocused targets with no original package files yet. |
| priority_lsms_raw_intake_blocked_requirement_rows | 80 | Raw-package acceptance requirements still blocked. |
...
| priority_lsms_archive_preflight_direct_file_rows | 972 | Direct non-generated original files detected by archive preflight. |
| priority_lsms_archive_preflight_archive_member_rows | 52 | Readable archive member rows detected without extraction. |
| priority_lsms_archive_preflight_blocked_dataset_rows | 9 | Refocused targets still blocked before schema/manual raw review. |
| priority_lsms_archive_preflight_blocked_requirement_rows | 72 | Archive preflight requirement rows still blocked. |
...
| priority_country_wave_packet_passed_gate_rows | 52 | Priority packet gates currently passing. |
...
| priority_country_wave_packet_endpoint_matrix_ready_rows | 13 | Priority packets with official endpoint matrix completed. |
| priority_country_wave_packet_credentialed_acquisition_ready_rows | 13 | Priority packets with credentialed raw acquisition ledger prepared. |
...
| bundle_section_priority_bundle | 99 | Direct-read bundle section count. |
...
| bundle_status_acquisition_gap_board_10_packages_threshold_gap_locked | 1 | Direct-read bundle status count. |
...
| bundle_status_blocked_manual_verification_incomplete | 1 | Direct-read bundle status count. |
...
| bundle_status_blocked_no_local_package | 1 | Direct-read bundle status count. |
...
| bundle_status_dataset_scope_locked_6_countries_11_waves_10_downloads_required | 1 | Direct-read bundle status count. |
...
| bundle_status_download_acceptance_matrix_current | 1 | Direct-read bundle status count. |
| bundle_status_download_execution_ready_raw_missing | 1 | Direct-read bundle status count. |
| bundle_status_endpoint_refresh_needs_review | 1 | Direct-read bundle status count. |
| bundle_status_external_local_candidates_found_provenance_not_accepted | 1 | Direct-read bundle status count. |
...
| bundle_status_focused_raw_value_decision_packet_ready_fail_closed | 1 | Direct-read bundle status count. |
| bundle_status_harmonization_value_audit_required | 1 | Direct-read bundle status count. |
| bundle_status_household_join_readiness_ready_fail_closed | 1 | Direct-read bundle status count. |
...
| bundle_status_manual_download_packets_current | 1 | Direct-read bundle status count. |
| bundle_status_manual_download_progress_current | 1 | Direct-read bundle status count. |
...
| bundle_status_minimum_batch_climate_linkage_review_current | 1 | Direct-read bundle status count. |
| bundle_status_minimum_batch_intake_needs_review | 1 | Direct-read bundle status count. |
| bundle_status_minimum_batch_raw_package_actions_ready | 1 | Direct-read bundle status count. |
| bundle_status_minimum_batch_raw_value_queue_current | 1 | Direct-read bundle status count. |
| bundle_status_minimum_batch_unlock_board_current_all_raw_files_absent | 1 | Direct-read bundle status count. |
...
| bundle_status_mwi2004_sdg382_candidate_classification_precheck_current_gate_closed | 1 | Direct-read bundle status count. |
...
| bundle_status_raw_packages_staged_for_receipt_validation_provenance_not_accepted | 1 | Direct-read bundle status count. |
...
| bundle_status_ready_for_browser_manual_open | 1 | Direct-read bundle status count. |
...
| bundle_status_target_folder_receipt_smoke_current | 1 | Direct-read bundle status count. |
...
| bundle_status_threshold_gap_control_panel_current | 1 | Direct-read bundle status count. |
...
| bundle_status_waiting_for_incoming_packages | 1 | Direct-read bundle status count. |
...
| blocked_manual_verification_incomplete | 1 |
...
| focused_raw_value_decision_packet_ready_fail_closed | 1 |
| household_join_readiness_ready_fail_closed | 1 |
...
| mwi2004_sdg382_internal_inputs_ready_external_parameters_blocked | 1 |
...
| endpoint_refresh_needs_review | 1 |
...
| download_acceptance_matrix_current | 1 |
| local_target_readmes_current | 1 |
| minimum_batch_raw_value_queue_current | 1 |
| target_folder_receipt_smoke_current | 1 |
| threshold_replacement_plan_current | 1 |
| minimum_batch_climate_linkage_review_current | 1 |
...
| raw_packages_staged_for_receipt_validation_provenance_not_accepted | 1 |
...
| priority_manual_verification_decision_gate | blocked_manual_verification_incomplete | datasets=13; requirements=104; concepts=169; variables=1214; requirements_verified=0; concepts_verified=0; variables_... | Priority manual verification decision gate consumes preserved fill-field evidence from the workbook and blocks promot... |
...
| priority_credentialed_raw_acquisition_ledger | ready_for_credentialed_manual_download | datasets=13; priority_rows=10; backup_rows=3; full_files=965; core_files=156; public_docs_ready=13; metadata_ready=13... | Credentialed raw acquisition ledger gives the exact official get-microdata workflow, complete-package scope, target f... |

## Artifact Manifest

| Artifact status | Count |
|---|---:|
| present_nonempty | 922 |
| missing_or_empty | 4 |
