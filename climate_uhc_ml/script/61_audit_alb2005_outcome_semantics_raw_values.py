...
AUDIT_PATH = TEMP_DIR / "alb2005_outcome_semantics_raw_value_audit.csv"
SUMMARY_PATH = RESULT_DIR / "alb2005_outcome_semantics_raw_value_summary.csv"
REPORT_PATH = REPORT_DIR / "alb2005_outcome_semantics_raw_value_audit.md"
...
BLOCKING_REASON = (
    ...
    "timing, partial geography/no GPS, OOP unit and recall-period comparability, gift/payment scope, missing-code "
    "review, questionnaire skip-pattern denominators, SDG discretionary-budget inputs, and cross-wave comparability"
)
...
CANDIDATES = [
    ...
    *[
    ...
    },
    {
        ...
        "source_file": "Modul_9B_healthb_cl.sav",
    ...
    *[
        ...
        for var in ["m9b_q02", "m9b_q023", "m9b_q024", "m9b_q025", "m9b_q026"]
    ],
    {
...
def add_hhid_key(df: pd.DataFrame, hhid: str = "hhid") -> pd.DataFrame:
    out = df.copy()
...
def merge_status(df: pd.DataFrame, base_keys: set[str], source_unit: str) -> dict[str, str]:
    complete = df["hhid_key"].astype(str).str.len() > 0
    complete_count = int(complete.sum())
    distinct_keys = set(df.loc[complete, "hhid_key"].astype(str))
...
def build_audit() -> list[dict[str, str]]:
    ...
    base_keys = {key_part(value) for value in core["hhid"] if key_part(value)}
...
def build_summary(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    gift_rows = sum(1 for row in rows if "gift" in row["recall_or_semantics_evidence"].lower())
    return [
        summary_row("alb2005_outcome_semantics_raw_value_audit_rows", len(rows), "Rows in the ALB_2005 raw value/semantics audit."),
        summary_row("alb2005_outcome_semantics_source_files_scanned", len(source_files), "Raw health modules scanned for outcome semantics."),
        summary_row("alb2005_outcome_semantics_financial_oop_candidate_rows", sum(1 for row in rows if row["outcome_domain"].startswith("financial_oop")), "Raw OOP payment/gift candidate rows."),
        summary_row("alb2005_outcome_semantics_gift_candidate_rows", gift_rows, "Raw gift-value rows whose inclusion in OOP requires review."),
        summary_row("alb2005_outcome_semantics_access_candidate_rows", sum(1 for row in rows if row["outcome_domain"].startswith("access")), "Raw access/affordability candidate rows."),
        summary_row("alb2005_outcome_semantics_need_candidate_rows", sum(1 for row in rows if row["outcome_domain"] == "need_proxy"), "Raw health-need proxy rows."),
        summary_row("alb2005_outcome_semantics_coping_candidate_rows", sum(1 for row in rows if row["outcome_domain"] == "coping_proxy"), "Raw health-payment coping proxy rows."),
        summary_row("alb2005_outcome_semantics_rows_with_value_labels", with_value_labels, "Candidate rows with SPSS value-label metadata."),
        summary_row("alb2005_outcome_semantics_rows_with_positive_numeric_values", positive_numeric, "Candidate rows with at least one positive numeric value."),
        summary_row("alb2005_outcome_semantics_rows_with_complete_household_merge_coverage", complete_merge, "Candidate rows whose source module covers all base households."),
        summary_row("alb2005_outcome_semantics_conditional_reason_rows", conditional_reason, "Reason variables that are conditional on access-barrier events and need denominator review."),
        summary_row("alb2005_outcome_semantics_outcome_ready_rows", 0, "No ALB_2005 raw value/semantics row is ready for final outcome promotion."),
        summary_row("alb2005_outcome_semantics_sdg382_ready_rows", 0, "No ALB_2005 row establishes the SDG 3.8.2 discretionary-budget denominator."),
        summary_row("alb2005_outcome_semantics_climate_linkage_ready_rows", 0, "No ALB_2005 row has verified timing/geography for climate linkage."),
        summary_row("alb2005_outcome_semantics_current_decision", DECISION, "Current fail-closed decision for ALB_2005 raw outcome semantics."),
    ]


def markdown_rows(rows: list[dict[str, str]], columns: list[str], limit: int = 55) -> str:
...
def write_report(rows: list[dict[str, str]], summary: list[dict[str, str]]) -> None:
    ...
    report = f"""# ALB_2005 Outcome Semantics Raw Value Audit
...
{markdown_rows(rows, ['outcome_domain', 'candidate_variable', 'source_file', 'raw_variable', 'raw_label', 'nonmissing_rows', 'distinct_values', 'min_value', 'max_value', 'positive_numeric_rows', 'value_label_examples', 'skip_pattern_status', 'unit_status', 'merge_key_status', 'promotion_status'], 60)}
...
def main() -> None:
    ...
    append_log(TEMP_DIR / "audit_log.md", f"Built ALB_2005 outcome semantics raw value audit rows={len(rows)}; decision={DECISION}.")
    print(f"ALB_2005 outcome semantics raw value audit rows={len(rows)}; decision={DECISION}.")
