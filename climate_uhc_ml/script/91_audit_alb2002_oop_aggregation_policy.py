...
AUDIT_PATH = TEMP_DIR / "alb2002_oop_aggregation_policy_audit.csv"
SUMMARY_PATH = RESULT_DIR / "alb2002_oop_aggregation_policy_summary.csv"
REPORT_PATH = REPORT_DIR / "alb2002_oop_aggregation_policy_audit.md"
...
FOUR_WEEK_PROVIDER_BASE = ["m5a_q14", "m5a_q24", "m5a_q32", "m5a_q40"]
FOUR_WEEK_GIFTS = ["m5a_q15", "m5a_q25", "m5a_q33", "m5a_q41"]
FOUR_WEEK_MEDICINES = ["m5a_q18", "m5a_q27", "m5a_q35", "m5a_q43", "m5a_q47"]
FOUR_WEEK_LAB = ["m5a_q20", "m5a_q28", "m5a_q36", "m5a_q44"]
FOUR_WEEK_TRANSPORT = ["m5a_q21", "m5a_q29", "m5a_q37", "m5a_q45"]

TWELVE_MONTH_PROVIDER_BASE = ["m5a_q53", "m5a_q61"]
TWELVE_MONTH_GIFTS = ["m5a_q54", "m5a_q62"]
TWELVE_MONTH_MEDICINES = ["m5a_q56", "m5a_q64"]
TWELVE_MONTH_LAB = ["m5a_q57", "m5a_q65"]
TWELVE_MONTH_TRANSPORT = ["m5a_q58", "m5a_q66"]
...
def build_candidate() -> pd.DataFrame:
    ...
    candidate = add_psu_hh_key(candidate, "psu", "hh")
    healtha = read_sav("Modul_5A_Health.sav", ["psu", "hh", *ALL_HEALTH_A_VARIABLES])
    ...
    grouped = healtha.groupby("psu_hh_key", dropna=False)[list(component_defs.keys())].sum().reset_index()
    merged = candidate.merge(grouped, on="psu_hh_key", how="left")
    for column in component_defs:
...
def audit_row(candidate: pd.DataFrame, spec: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "blocking_reason": (
            "Audit-only aggregation stress test. ALB_2002 still lacks final OOP inclusion policy, SDG 3.8.2 denominator inputs, "
            "recall-period comparability decision, zero/nonmissing skipped-payment review, and climate-ready district geography."
        ),
        "next_action": (
            "Use this table to choose and document an OOP inclusion policy only after consumption units, payment scope, "
            "gift/transport treatment, skip paths, SDG denominator inputs, and geography are verified."
        ),
    }
...
def build_summary(rows: list[dict[str, str]], candidate: pd.DataFrame) -> list[dict[str, str]]:
    ...
    no_gifts_with_transport_4w = (
        pd.to_numeric(candidate["oop_4w_provider_base"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_4w_medicines"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_4w_lab"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_4w_transport"], errors="coerce").fillna(0)
    )
    no_gifts_with_transport_12m = (
        pd.to_numeric(candidate["oop_12m_provider_base"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_12m_medicines"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_12m_lab"], errors="coerce").fillna(0)
        + pd.to_numeric(candidate["oop_12m_transport"], errors="coerce").fillna(0)
    )
    core_4w = pd.to_numeric(candidate["oop_4w_sum_unreviewed"], errors="coerce").fillna(0)
    core_12m = pd.to_numeric(candidate["oop_12m_sum_unreviewed"], errors="coerce").fillna(0)
    match_4w = (no_gifts_with_transport_4w.sub(core_4w).abs() < 1e-6).sum()
    match_12m = (no_gifts_with_transport_12m.sub(core_12m).abs() < 1e-6).sum()
    return [
        summary_row("alb2002_oop_aggregation_policy_rows", len(rows), "Rows in the ALB_2002 OOP aggregation policy audit."),
        summary_row("alb2002_oop_aggregation_policy_household_rows", len(candidate), "Base household rows included in the stress test."),
        summary_row("alb2002_oop_aggregation_policy_total_consumption_rows", int((pd.to_numeric(candidate["total_consumption"], errors="coerce") > 0).sum()), "Households with positive total consumption denominator."),
        summary_row("alb2002_oop_aggregation_policy_four_week_policy_rows", sum(1 for row in rows if row["recall_scope"] == "past_4_weeks"), "Four-week OOP aggregation policies."),
        summary_row("alb2002_oop_aggregation_policy_twelve_month_policy_rows", sum(1 for row in rows if row["recall_scope"] == "past_12_months_hospital_dentist"), "Twelve-month hospital/dentist OOP aggregation policies."),
        summary_row("alb2002_oop_aggregation_policy_annual_stress_rows", len(annual_rows), "Annualized stress-test policies combining four-week and 12-month items."),
        summary_row("alb2002_oop_aggregation_policy_max_che10_rate", max_che10, "Maximum unweighted CHE10 stress-test rate across policies."),
        summary_row("alb2002_oop_aggregation_policy_max_che25_rate", max_che25, "Maximum unweighted CHE25 stress-test rate across policies."),
        summary_row("alb2002_oop_aggregation_policy_core_4w_match_rows", int(match_4w), "Rows where recomputed four-week no-gifts-with-transport OOP equals the existing core candidate sum."),
        summary_row("alb2002_oop_aggregation_policy_core_12m_match_rows", int(match_12m), "Rows where recomputed 12-month no-gifts-with-transport OOP equals the existing core candidate sum."),
        summary_row("alb2002_oop_aggregation_policy_questionnaire_oop_item_rows_observed", metric_value(questionnaire_summary, "alb2002_health_questionnaire_oop_item_rows"), "Questionnaire OOP item rows observed upstream."),
        summary_row("alb2002_oop_aggregation_policy_questionnaire_gift_item_rows_observed", metric_value(questionnaire_summary, "alb2002_health_questionnaire_gift_item_rows"), "Questionnaire gift/payment-scope rows observed upstream."),
        summary_row("alb2002_oop_aggregation_policy_questionnaire_new_lek_rows_observed", metric_value(questionnaire_summary, "alb2002_health_questionnaire_new_lek_unit_rows"), "Questionnaire NEW LEKS unit rows observed upstream."),
        summary_row("alb2002_oop_aggregation_policy_questionnaire_payment_positive_skip_leaks_observed", metric_value(questionnaire_summary, "alb2002_health_questionnaire_payment_positive_when_not_triggered_rows"), "Positive skipped-payment downstream rows observed upstream."),
        summary_row("alb2002_oop_aggregation_policy_questionnaire_payment_nonmissing_skip_review_observed", metric_value(questionnaire_summary, "alb2002_health_questionnaire_payment_nonmissing_when_not_triggered_rows"), "Nonmissing skipped-payment downstream rows requiring review upstream."),
        summary_row("alb2002_oop_aggregation_policy_minimum_recipe_harmonized_ready_observed", metric_value(minimum_recipe_summary, "alb2002_minimum_recipe_promotion_harmonized_ready_rows"), "Harmonized-ready rows observed in the ALB_2002 minimum recipe packet."),
        summary_row("alb2002_oop_aggregation_policy_minimum_recipe_outcome_ready_observed", metric_value(minimum_recipe_summary, "alb2002_minimum_recipe_promotion_outcome_ready_rows"), "Outcome-ready rows observed in the ALB_2002 minimum recipe packet."),
        summary_row("alb2002_oop_aggregation_policy_climate_ready_rows_observed", metric_value(crosswalk_summary, "alb2002_climate_linkage_ready_rows"), "Climate-linkage-ready rows observed in the district crosswalk audit."),
        summary_row("alb2002_oop_aggregation_policy_outcome_ready_rows", 0, "Rows promoted to final outcome construction by this audit; intentionally zero."),
        summary_row("alb2002_oop_aggregation_policy_recipe_ready_rows", 0, "Rows promoted to harmonization recipe by this audit; intentionally zero."),
        summary_row("alb2002_oop_aggregation_policy_sdg382_ready_rows", 0, "Rows promoted to SDG 3.8.2 construction by this audit; intentionally zero."),
        summary_row("alb2002_oop_aggregation_policy_climate_linkage_ready_rows", 0, "Rows promoted to climate linkage by this audit; intentionally zero."),
        summary_row("alb2002_oop_aggregation_policy_current_decision", DECISION, "Current fail-closed decision for ALB_2002 OOP aggregation stress tests."),
    ]
...
def write_report(rows: list[dict[str, str]], summary: list[dict[str, str]]) -> None:
    ...
    report = f"""# ALB_2002 OOP Aggregation Policy Audit
...
def main() -> None:
    ...
    append_log(TEMP_DIR / "audit_log.md", f"Built ALB_2002 OOP aggregation policy audit rows={len(rows)} decision={DECISION}.")
    print(f"ALB_2002 OOP aggregation policy audit rows={len(rows)} decision={DECISION}.")
