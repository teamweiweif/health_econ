...
def validate_artifacts(rows: list[dict[str, Any]]) -> None:
    counts = {
        ...
        "priority_lsms_isa_worldbank_session_bootstrap": row_count(RESULT_DIR / "priority_lsms_isa_worldbank_session_bootstrap.csv"),
        ...
        "priority_lsms_isa_package_level_download_manifest": row_count(RESULT_DIR / "priority_lsms_isa_package_level_download_manifest.csv"),
        "priority_lsms_isa_package_level_core_file_manifest": row_count(RESULT_DIR / "priority_lsms_isa_package_level_core_file_manifest.csv"),
        "priority_lsms_isa_package_level_download_manifest_summary": row_count(RESULT_DIR / "priority_lsms_isa_package_level_download_manifest_summary.csv"),
        "priority_lsms_isa_acquisition_gap_receipt_board": row_count(RESULT_DIR / "priority_lsms_isa_acquisition_gap_receipt_board.csv"),
        "priority_lsms_isa_acquisition_gap_receipt_summary": row_count(RESULT_DIR / "priority_lsms_isa_acquisition_gap_receipt_summary.csv"),
    ...
    priority_lsms_worldbank_session_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_bootstrap_rows"), "0"), 0)
    priority_lsms_worldbank_session_cookie_present = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_cookie_file_present"), "")
    priority_lsms_worldbank_session_cookie_status = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_cookie_format_status"), "")
    priority_lsms_worldbank_session_header_present = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_header_file_present"), "")
    priority_lsms_worldbank_session_header_status = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_header_format_status"), "")
    priority_lsms_worldbank_session_ready = safe_int(next((row.get("value", "0") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_bootstrap_ready_for_probe_rows"), "0"), 0)
    priority_lsms_worldbank_session_missing = safe_int(next((row.get("value", "0") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "worldbank_session_bootstrap_missing_session_rows"), "0"), 0)
    priority_lsms_worldbank_session_data_write = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "data_write_gate_status"), "")
    priority_lsms_worldbank_session_modeling = next((row.get("value", "") for row in priority_lsms_worldbank_session_summary if row.get("metric") == "modeling_gate_status"), "")
    priority_lsms_worldbank_session_gate_ok = (
        counts["priority_lsms_isa_worldbank_session_bootstrap_summary"] > 0
        and counts["priority_lsms_isa_worldbank_session_bootstrap"] == priority_lsms_worldbank_session_rows
        and file_ok(REPORT_DIR / "priority_lsms_isa_worldbank_session_bootstrap.md")
        and priority_lsms_worldbank_session_rows == 10
        and priority_lsms_worldbank_session_ready + priority_lsms_worldbank_session_missing == priority_lsms_worldbank_session_rows
        and priority_lsms_worldbank_session_cookie_present in {"0", "1"}
        and priority_lsms_worldbank_session_header_present in {"0", "1"}
        and priority_lsms_worldbank_session_cookie_status != ""
        and priority_lsms_worldbank_session_header_status != ""
        and priority_lsms_worldbank_session_data_write == "blocked_no_data_write"
        and priority_lsms_worldbank_session_modeling == "blocked"
    )
    add(
        rows,
        "dataset_promotion",
        "Priority LSMS/ISA World Bank session bootstrap redacts local credential readiness for the 10 credentialed raw-package downloads",
        status(priority_lsms_worldbank_session_gate_ok),
        f"bootstrap_rows={counts['priority_lsms_isa_worldbank_session_bootstrap']}; summary_rows={counts['priority_lsms_isa_worldbank_session_bootstrap_summary']}; cookie_present={priority_lsms_worldbank_session_cookie_present}; cookie_status={priority_lsms_worldbank_session_cookie_status}; header_present={priority_lsms_worldbank_session_header_present}; header_status={priority_lsms_worldbank_session_header_status}; ready_for_probe={priority_lsms_worldbank_session_ready}; missing_session={priority_lsms_worldbank_session_missing}; data_write={priority_lsms_worldbank_session_data_write}; modeling_gate={priority_lsms_worldbank_session_modeling}",
        "" if priority_lsms_worldbank_session_gate_ok else "Run script/194_build_priority_lsms_isa_worldbank_session_bootstrap.py after the minimum-batch promotion unlock board and credentialed download handoff are current.",
    )
    ...
    priority_lsms_fetch_command_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_rows"), "0"), 0)
    priority_lsms_fetch_command_countries = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_countries"), "0"), 0)
    priority_lsms_fetch_command_expected_files = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_expected_core_file_rows"), "0"), 0)
    priority_lsms_fetch_command_target_files = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_target_file_count"), "0"), 0)
    priority_lsms_fetch_command_ready = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_ready_to_probe_rows"), "0"), 0)
    priority_lsms_fetch_command_missing = safe_int(next((row.get("value", "0") for row in priority_lsms_fetch_command_summary if row.get("metric") == "credentialed_fetch_command_packet_missing_session_rows"), "0"), 0)
    priority_lsms_fetch_command_data_write = next((row.get("value", "") for row in priority_lsms_fetch_command_summary if row.get("metric") == "data_write_gate_status"), "")
    priority_lsms_fetch_command_modeling = next((row.get("value", "") for row in priority_lsms_fetch_command_summary if row.get("metric") == "modeling_gate_status"), "")
    priority_lsms_fetch_command_gate_ok = (
        counts["priority_lsms_isa_credentialed_fetch_command_packet_summary"] > 0
        and counts["priority_lsms_isa_credentialed_fetch_command_packet"] == priority_lsms_fetch_command_rows
        and file_ok(REPORT_DIR / "priority_lsms_isa_credentialed_fetch_command_packet.md")
        and priority_lsms_fetch_command_rows == 10
        and priority_lsms_fetch_command_countries >= 5
        and priority_lsms_fetch_command_expected_files > 0
        and priority_lsms_fetch_command_target_files >= 0
        and priority_lsms_fetch_command_ready + priority_lsms_fetch_command_missing == priority_lsms_fetch_command_rows
        and priority_lsms_fetch_command_data_write == "blocked_no_data_write"
        and priority_lsms_fetch_command_modeling == "blocked"
    )
    add(
        rows,
        "dataset_promotion",
        "Priority LSMS/ISA credentialed fetch command packet maps the 10 minimum-batch downloads to target payload paths and validation commands",
    ...
    priority_lsms_package_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_manifest_rows"), "0"), 0)
    priority_lsms_package_countries = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_country_rows"), "0"), 0)
    priority_lsms_package_priority_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_priority_country_rows"), "0"), 0)
    priority_lsms_package_sixth_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_sixth_country_rows"), "0"), 0)
    priority_lsms_package_full_files = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_expected_full_file_rows"), "0"), 0)
    priority_lsms_package_core_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_expected_core_file_rows"), "0"), 0)
    priority_lsms_package_unique_core = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_unique_core_file_manifest_rows"), "0"), 0)
    priority_lsms_package_requirement_gates = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_requirement_gate_rows"), "0"), 0)
    priority_lsms_package_blocked_requirements = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_blocked_requirement_gate_rows"), "0"), 0)
    priority_lsms_package_target_files = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_target_file_rows"), "0"), 0)
    priority_lsms_package_incoming_files = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_incoming_file_rows"), "0"), 0)
    priority_lsms_package_blocked_no_local = safe_int(next((row.get("value", "0") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_blocked_no_local_package_rows"), "0"), 0)
    priority_lsms_package_canary = next((row.get("value", "") for row in priority_lsms_package_manifest_summary if row.get("metric") == "package_level_download_first_canary_idno"), "")
    priority_lsms_package_data_write = next((row.get("value", "") for row in priority_lsms_package_manifest_summary if row.get("metric") == "data_write_gate_status"), "")
    priority_lsms_package_modeling = next((row.get("value", "") for row in priority_lsms_package_manifest_summary if row.get("metric") == "modeling_gate_status"), "")
    priority_lsms_package_manifest_gate_ok = (
        counts["priority_lsms_isa_package_level_download_manifest"] == priority_lsms_package_rows
        and counts["priority_lsms_isa_package_level_core_file_manifest"] == priority_lsms_package_unique_core
        and counts["priority_lsms_isa_package_level_download_manifest_summary"] > 0
        and file_ok(REPORT_DIR / "priority_lsms_isa_package_level_download_manifest.md")
        and priority_lsms_package_rows == 10
        and priority_lsms_package_countries == 5
        and priority_lsms_package_priority_rows == 9
        and priority_lsms_package_sixth_rows == 1
        and priority_lsms_package_full_files == 838
        and priority_lsms_package_core_rows == 323
        and priority_lsms_package_unique_core == 213
        and priority_lsms_package_requirement_gates == 70
        and priority_lsms_package_blocked_requirements == 70
        and priority_lsms_package_target_files == 0
        and priority_lsms_package_incoming_files == 0
        and priority_lsms_package_blocked_no_local == 10
        and priority_lsms_package_canary == "ETH_2021_ESPS-W5_v02_M"
        and priority_lsms_package_data_write == "blocked_no_data_write"
        and priority_lsms_package_modeling == "blocked"
    )
    add(
        rows,
        "dataset_promotion",
        "Priority LSMS/ISA package-level download manifest compresses the 10 World Bank downloads into package receipt actions",
        status(priority_lsms_package_manifest_gate_ok),
        f"rows={priority_lsms_package_rows}; countries={priority_lsms_package_countries}; priority_rows={priority_lsms_package_priority_rows}; sixth_country_rows={priority_lsms_package_sixth_rows}; expected_full_files={priority_lsms_package_full_files}; expected_core_rows={priority_lsms_package_core_rows}; unique_core_files={priority_lsms_package_unique_core}; requirement_gates={priority_lsms_package_requirement_gates}; blocked_requirements={priority_lsms_package_blocked_requirements}; target_files={priority_lsms_package_target_files}; incoming_files={priority_lsms_package_incoming_files}; blocked_no_local_package={priority_lsms_package_blocked_no_local}; first_canary={priority_lsms_package_canary}; data_write={priority_lsms_package_data_write}; modeling_gate={priority_lsms_package_modeling}",
        "" if priority_lsms_package_manifest_gate_ok else "Run script/206_build_priority_lsms_isa_package_level_download_manifest.py after the manual launchpad and download acceptance matrix are current.",
    )
    priority_lsms_acquisition_gap_summary = read_csv_dicts(RESULT_DIR / "priority_lsms_isa_acquisition_gap_receipt_summary.csv")
    priority_lsms_acquisition_gap_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_receipt_board_rows"), "0"), 0)
    priority_lsms_acquisition_gap_countries = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_receipt_country_rows"), "0"), 0)
    priority_lsms_acquisition_gap_current_promoted = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "current_promoted_analysis_ready_rows"), "0"), 0)
    priority_lsms_acquisition_gap_current_financial = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "current_financial_protection_ready_countries"), "0"), 0)
    priority_lsms_acquisition_gap_current_double = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "current_double_failure_ready_country_waves"), "0"), 0)
    priority_lsms_acquisition_gap_financial_gap = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "remaining_financial_country_gap"), "0"), 0)
    priority_lsms_acquisition_gap_double_gap = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "remaining_double_failure_wave_gap"), "0"), 0)
    priority_lsms_acquisition_gap_missing_expected = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_missing_expected_file_rows"), "0"), 0)
    priority_lsms_acquisition_gap_missing_core = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_missing_core_file_rows"), "0"), 0)
    priority_lsms_acquisition_gap_blocked_requirements = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_blocked_requirement_rows"), "0"), 0)
    priority_lsms_acquisition_gap_target_files = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_target_file_rows"), "0"), 0)
    priority_lsms_acquisition_gap_incoming_files = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_incoming_file_rows"), "0"), 0)
    priority_lsms_acquisition_gap_missing_packages = safe_int(next((row.get("value", "0") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_missing_package_rows"), "0"), 0)
    priority_lsms_acquisition_gap_canary = next((row.get("value", "") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "acquisition_gap_first_canary_idno"), "")
    priority_lsms_acquisition_gap_data_write = next((row.get("value", "") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "data_write_gate_status"), "")
    priority_lsms_acquisition_gap_modeling = next((row.get("value", "") for row in priority_lsms_acquisition_gap_summary if row.get("metric") == "modeling_gate_status"), "")
    priority_lsms_acquisition_gap_gate_ok = (
        counts["priority_lsms_isa_acquisition_gap_receipt_board"] == priority_lsms_acquisition_gap_rows
        and counts["priority_lsms_isa_acquisition_gap_receipt_summary"] > 0
        and file_ok(REPORT_DIR / "priority_lsms_isa_acquisition_gap_receipt_board.md")
        and priority_lsms_acquisition_gap_rows == 10
        and priority_lsms_acquisition_gap_countries == 5
        and priority_lsms_acquisition_gap_current_promoted == 1
        and priority_lsms_acquisition_gap_current_financial == 1
        and priority_lsms_acquisition_gap_current_double == 1
        and priority_lsms_acquisition_gap_financial_gap == 5
        and priority_lsms_acquisition_gap_double_gap == 9
        and priority_lsms_acquisition_gap_missing_expected == 838
        and priority_lsms_acquisition_gap_missing_core == 323
        and priority_lsms_acquisition_gap_blocked_requirements == 70
        and priority_lsms_acquisition_gap_target_files == 0
        and priority_lsms_acquisition_gap_incoming_files == 0
        and priority_lsms_acquisition_gap_missing_packages == 10
        and priority_lsms_acquisition_gap_canary == "ETH_2021_ESPS-W5_v02_M"
        and priority_lsms_acquisition_gap_data_write == "blocked_no_data_write"
        and priority_lsms_acquisition_gap_modeling == "blocked"
    )
    add(
        rows,
        "dataset_promotion",
        "Priority LSMS/ISA acquisition gap receipt board links remaining raw-package downloads to threshold gaps and validation commands",
        status(priority_lsms_acquisition_gap_gate_ok),
        f"rows={priority_lsms_acquisition_gap_rows}; countries={priority_lsms_acquisition_gap_countries}; current_promoted={priority_lsms_acquisition_gap_current_promoted}; current_financial_countries={priority_lsms_acquisition_gap_current_financial}; current_double_failure_waves={priority_lsms_acquisition_gap_current_double}; financial_gap={priority_lsms_acquisition_gap_financial_gap}; double_failure_gap={priority_lsms_acquisition_gap_double_gap}; missing_expected_files={priority_lsms_acquisition_gap_missing_expected}; missing_core_files={priority_lsms_acquisition_gap_missing_core}; blocked_requirements={priority_lsms_acquisition_gap_blocked_requirements}; target_files={priority_lsms_acquisition_gap_target_files}; incoming_files={priority_lsms_acquisition_gap_incoming_files}; missing_packages={priority_lsms_acquisition_gap_missing_packages}; first_canary={priority_lsms_acquisition_gap_canary}; data_write={priority_lsms_acquisition_gap_data_write}; modeling_gate={priority_lsms_acquisition_gap_modeling}",
        "" if priority_lsms_acquisition_gap_gate_ok else "Run script/208_build_priority_lsms_isa_acquisition_gap_receipt_board.py after the package-level manifest, post-download receipt handoff, and promotion registry are current.",
    )
    ...
    priority_lsms_external_candidate_rows = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_queue_rows"), "0"), 0)
    priority_lsms_external_candidate_root_exists = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_root_exists"), "0"), 0)
    priority_lsms_external_candidate_folder_exists = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_folder_exists_rows"), "0"), 0)
    priority_lsms_external_candidate_expected_matches = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_expected_match_rows"), "0"), 0)
    priority_lsms_external_candidate_core_matches = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_core_match_rows"), "0"), 0)
    priority_lsms_external_candidate_locked_targets = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_locked_target_rows"), "0"), 0)
    priority_lsms_external_candidate_locked_with_matches = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_locked_target_with_matches_rows"), "0"), 0)
    priority_lsms_external_candidate_locked_core_complete = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_locked_core_complete_rows"), "0"), 0)
    priority_lsms_external_candidate_backup_with_matches = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_backup_with_matches_rows"), "0"), 0)
    priority_lsms_external_candidate_provenance_accepted = safe_int(next((row.get("value", "0") for row in priority_lsms_external_candidate_summary if row.get("metric") == "external_local_raw_candidate_provenance_accepted_rows"), "0"), 0)
    priority_lsms_external_candidate_data_write = next((row.get("value", "") for row in priority_lsms_external_candidate_summary if row.get("metric") == "data_write_gate_status"), "")
    priority_lsms_external_candidate_modeling = next((row.get("value", "") for row in priority_lsms_external_candidate_summary if row.get("metric") == "modeling_gate_status"), "")
    priority_lsms_external_candidate_gate_ok = (
        counts["priority_lsms_isa_external_local_raw_candidate_audit"] == priority_lsms_external_candidate_rows
        and counts["priority_lsms_isa_external_local_raw_candidate_file_match"] >= 1500
        and counts["priority_lsms_isa_external_local_raw_candidate_summary"] > 0
        and file_ok(REPORT_DIR / "priority_lsms_isa_external_local_raw_candidate_audit.md")
        and priority_lsms_external_candidate_rows == 19
        and priority_lsms_external_candidate_root_exists == 1
        and priority_lsms_external_candidate_folder_exists >= 10
        and priority_lsms_external_candidate_expected_matches >= 800
        and priority_lsms_external_candidate_core_matches >= 280
        and priority_lsms_external_candidate_locked_targets == 13
        and priority_lsms_external_candidate_locked_with_matches >= 7
        and priority_lsms_external_candidate_locked_core_complete >= 4
        and priority_lsms_external_candidate_backup_with_matches >= 3
        and priority_lsms_external_candidate_provenance_accepted == 0
        and priority_lsms_external_candidate_data_write == "blocked_no_data_write"
        and priority_lsms_external_candidate_modeling == "blocked"
    )
    add(
        rows,
        "dataset_promotion",
        "Priority LSMS/ISA external local raw candidate audit finds acquisition leads but does not accept provenance",
