...
AUDIT_PATH = TEMP_DIR / "alb2008_provisional_outcome_feasibility_audit.csv"
SUMMARY_PATH = RESULT_DIR / "alb2008_provisional_outcome_feasibility_summary.csv"
REPORT_PATH = REPORT_DIR / "alb2008_provisional_outcome_feasibility.md"
...
BLOCKING_REASON = (
    ...
    "SDG discretionary denominator inputs, units, missing-code semantics, access skip patterns, and "
    "cross-wave comparability remain unresolved"
)
...
def build_audit(df: pd.DataFrame) -> list[dict[str, str]]:
    ...
    delayed_or_referral = combine_any(
        [
    ...
    rows.append(
        event_row(
            ...
            "0.10",
            "oop_12m_sum_unreviewed / total_consumption > 0.10 OR delayed/referral/refusal proxy",
            valid_total,
    ...
    rows.append(
        event_row(
            ...
            "0.25",
            "oop_12m_sum_unreviewed / total_consumption > 0.25 OR delayed/referral/refusal proxy",
            valid_total,
...
def build_summary(df: pd.DataFrame, audit_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    weight = num(df, "household_weight")
    rows = [
        summary_row("alb2008_provisional_outcome_audit_rows", len(audit_rows), "Rows in the provisional ALB_2008 outcome feasibility audit."),
        summary_row(
            "alb2008_provisional_financial_stress_test_rows",
            sum(1 for row in audit_rows if row["outcome_family"] == "financial_stress_test"),
            "Financial-protection stress-test rows, none final.",
        ),
        summary_row(
            "alb2008_provisional_access_proxy_rows",
            sum(1 for row in audit_rows if row["outcome_family"] == "access_proxy"),
            "Access proxy rows, none final.",
        ),
        summary_row(
            "alb2008_provisional_low_event_rate_rows",
            sum(1 for row in audit_rows if row["low_event_rate_flag"] == "1"),
            "Outcome candidates with unweighted event rate below 3 percent.",
        ),
        summary_row("alb2008_provisional_outcome_base_rows", len(df), "Rows in temp/alb2008_household_core_candidate.csv."),
        summary_row("alb2008_provisional_positive_total_consumption_rows", int(((total.notna()) & (total > 0)).sum()), "Rows with positive total_consumption."),
        summary_row("alb2008_provisional_positive_household_weight_rows", int(((weight.notna()) & (weight > 0)).sum()), "Rows with positive household_weight."),
        summary_row(
            "alb2008_provisional_consumption_weight_rows",
            int(((total.notna()) & (total > 0) & (weight.notna()) & (weight > 0)).sum()),
            "Rows with both positive total_consumption and positive household_weight.",
        ),
        summary_row("alb2008_provisional_outcome_ready_rows", 0, "No provisional ALB_2008 diagnostic row is ready for data/ or outcome promotion."),
        summary_row("alb2008_provisional_outcome_current_decision", DECISION, "Current fail-closed decision for ALB_2008 provisional outcome feasibility."),
    ]
    return rows
...
def write_report(audit_rows: list[dict[str, str]], summary_rows: list[dict[str, str]]) -> None:
    ...
    REPORT_PATH.write_text(
        f"""# ALB_2008 Provisional Outcome Feasibility Audit
...
def main() -> None:
    ...
    append_log(
        ...
        f"Built ALB_2008 provisional outcome feasibility audit rows={len(audit_rows)} decision={DECISION}.",
    )
    print(f"ALB_2008 provisional outcome feasibility audit rows={len(audit_rows)} decision={DECISION}.")
