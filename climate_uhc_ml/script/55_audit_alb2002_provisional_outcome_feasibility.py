...
AUDIT_PATH = TEMP_DIR / "alb2002_provisional_outcome_feasibility_audit.csv"
SUMMARY_PATH = RESULT_DIR / "alb2002_provisional_outcome_feasibility_summary.csv"
REPORT_PATH = REPORT_DIR / "alb2002_provisional_outcome_feasibility.md"
...
BLOCKING_REASON = (
    ...
    "units, missing-code semantics, access skip patterns, district climate crosswalk, no-GPS geography, "
    "fieldwork documentation, and cross-wave comparability remain unresolved"
)
DECISION = "not_final_outcomes_outcome_semantics_climate_crosswalk_blocked"
...
def build_audit(df: pd.DataFrame) -> list[dict[str, str]]:
    ...
    for threshold in [0.10, 0.25]:
        ...
        annualized_share_4w = (oop_4w * 13) / total
        rows.append(
            event_row(
                df,
                "financial_stress_test",
    ...
    for name in ["illness_or_disability_any", "sudden_illness_4w_any"]:
        add_bool_row(rows, df, "need_proxy", name, f"{name} == 1", "need_proxy_unreviewed_not_outcome")

    oop12_che10 = (oop_12m / total) > 0.10
    oop12_che25 = (oop_12m / total) > 0.25
    oop4w_ann_che10 = ((oop_4w * 13) / total) > 0.10
    rows.append(
        event_row(
            df,
            "composite_proxy",
...
def build_summary(df: pd.DataFrame, audit_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    total = num(df, "total_consumption")
    weight = num(df, "household_weight")
    district = num(df, "district_code_identification")
    survey_month = num(df, "survey_month")
    interview_date = df["interview_date"].astype(str) if "interview_date" in df.columns else pd.Series("", index=df.index)
    rows = [
        summary_row("alb2002_provisional_outcome_audit_rows", len(audit_rows), "Rows in the provisional ALB_2002 outcome feasibility audit."),
        summary_row(
            "alb2002_provisional_financial_stress_test_rows",
        ...
        summary_row(
            "alb2002_provisional_need_proxy_rows",
            sum(1 for row in audit_rows if row["outcome_family"] == "need_proxy"),
            "Need proxy rows, none final.",
        ),
        summary_row(
            "alb2002_provisional_low_event_rate_rows",
            sum(1 for row in audit_rows if row["low_event_rate_flag"] == "1"),
            "Outcome candidates with unweighted event rate below 3 percent.",
        ),
        summary_row("alb2002_provisional_outcome_base_rows", len(df), "Rows in temp/alb2002_household_core_candidate.csv."),
        summary_row("alb2002_provisional_positive_total_consumption_rows", int(((total.notna()) & (total > 0)).sum()), "Rows with positive total_consumption."),
        summary_row("alb2002_provisional_positive_household_weight_rows", int(((weight.notna()) & (weight > 0)).sum()), "Rows with positive household_weight."),
        summary_row(
            "alb2002_provisional_consumption_weight_rows",
            int(((total.notna()) & (total > 0) & (weight.notna()) & (weight > 0)).sum()),
            "Rows with both positive total_consumption and positive household_weight.",
        ),
        summary_row("alb2002_provisional_district_code_rows", int(district.notna().sum()), "Rows with district_code_identification for future crosswalk review."),
        summary_row("alb2002_provisional_survey_month_rows", int(survey_month.notna().sum()), "Rows with raw survey month for future climate window review."),
        summary_row("alb2002_provisional_interview_date_rows", int(interview_date.str.len().gt(0).sum()), "Rows with constructed raw interview date for future climate window review."),
        summary_row("alb2002_provisional_outcome_ready_rows", 0, "No provisional ALB_2002 diagnostic row is ready for data/ or outcome promotion."),
        summary_row("alb2002_provisional_outcome_current_decision", DECISION, "Current fail-closed decision for ALB_2002 provisional outcome feasibility."),
    ]
    return rows
...
def write_report(audit_rows: list[dict[str, str]], summary_rows: list[dict[str, str]]) -> None:
    ...
    REPORT_PATH.write_text(
        f"""# ALB_2002 Provisional Outcome Feasibility Audit
...
{markdown_rows(audit_rows, ['outcome_family', 'outcome_candidate', 'denominator_rows', 'numerator_rows', 'unweighted_rate', 'weighted_rate', 'low_event_rate_flag', 'promotion_status'], 40)}
...
def main() -> None:
    ...
    append_log(
        ...
        f"Built ALB_2002 provisional outcome feasibility audit rows={len(audit_rows)} decision={DECISION}.",
    )
    print(f"ALB_2002 provisional outcome feasibility audit rows={len(audit_rows)} decision={DECISION}.")
