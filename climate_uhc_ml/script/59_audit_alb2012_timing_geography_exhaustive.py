...
AUDIT_PATH = TEMP_DIR / "alb2012_timing_geography_exhaustive_audit.csv"
SUMMARY_PATH = RESULT_DIR / "alb2012_timing_geography_exhaustive_summary.csv"
REPORT_PATH = REPORT_DIR / "alb2012_timing_geography_exhaustive_audit.md"
...
DECISION = "blocked_missing_interview_timing_coarse_prefecture_region_no_gps"
...
def classify_candidate(source_file: str, variable_name: str, label: str) -> tuple[str, str, str, str]:
    ...
    if source_file == "poverty.sav" and name in {"prefecture", "region", "urban"}:
        return (
            "coarse_full_coverage_geography_candidate",
            "coarse prefecture/region/urban field in poverty.sav",
    ...
    if name in {"psu", "psu_key"} or lab.strip() == "psu":
        return (
            ...
            "PSU may support merges but cannot support climate linkage without coordinates or official admin crosswalk",
        )
    ...
    if any(term in blob for term in ["past 4 weeks", "past month", "past 30 days", "past 6 months", "past 12 months", "last 6 months", "recall"]):
        return (
    ...
    if any(term in blob for term in ["district", "city", "municipality", "commune", "village", "urban", "rural", "region", "area", "stratum", "prefecture"]):
        return (
...
def build_summary(rows: list[dict[str, str]], source_file_count: int, read_failures: int) -> list[dict[str, str]]:
    coarse_rows = [row for row in rows if row.get("candidate_role") == "coarse_full_coverage_geography_candidate"]
    coarse_household_rows = max([int(row.get("nonmissing_rows", "0") or 0) for row in coarse_rows], default=0)
    coordinate_rows = count_role(rows, "coordinate_candidate")
    survey_timing_candidates = count_role(rows, "survey_timing_candidate_requires_context")
    ...
    return [
        summary_row("alb2012_timing_geography_audit_rows", len(rows), "Timing/geography keyword rows scanned from ALB_2012 raw files."),
        summary_row("alb2012_timing_geography_source_files_scanned", source_file_count, "ALB_2012 .sav files scanned."),
        summary_row("alb2012_timing_geography_read_failures", read_failures, "Raw files that could not be read by pyreadstat."),
        summary_row("alb2012_interview_timing_candidate_rows", survey_timing_candidates, "Rows with apparent interview/fieldwork timing wording requiring context."),
        summary_row("alb2012_interview_timing_verified_rows", 0, "Verified household interview month/date rows available for climate linkage."),
        summary_row("alb2012_rejected_non_interview_timing_rows", rejected_timing, "Rows rejected as birth, recall, duration, payment-period, or event-history timing."),
        summary_row("alb2012_rejected_migration_history_rows", rejected_history, "Rows rejected as migration or historical residence geography/timing."),
        summary_row("alb2012_context_review_geography_timing_rows", context_review, "Rows with timing/geography keywords requiring context but not verified for climate linkage."),
        summary_row("alb2012_psu_cluster_key_rows", psu_rows, "PSU/cluster-like key rows observed."),
        summary_row("alb2012_coordinate_candidate_rows", coordinate_rows, "GPS/latitude/longitude/coordinate candidate rows observed."),
        summary_row("alb2012_coarse_full_coverage_geography_candidate_rows", len(coarse_rows), "Rows for full-coverage coarse geography/design fields in poverty.sav."),
        summary_row("alb2012_coarse_geography_household_rows", coarse_household_rows, "Maximum nonmissing household rows across coarse poverty.sav geography/design fields."),
        summary_row("alb2012_geography_verified_full_coverage_rows", 0, "Verified admin/GPS geography rows ready for climate linkage."),
        summary_row("alb2012_climate_linkage_ready_rows", 0, "Rows ready to support climate linkage input construction."),
        summary_row("alb2012_timing_geography_current_decision", DECISION, "Current ALB_2012 timing/geography decision."),
    ]


def build_audit() -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    if not RAW_ROOT.exists():
        raise FileNotFoundError(f"Missing ALB_2012 raw directory: {RAW_ROOT}")
    ...
    for path in source_files:
        ...
        except Exception as exc:  # pragma: no cover - raw audit records unusual failures.
            read_failures += 1
            rows.append(
                {
                    "country": COUNTRY,
...
def write_report(rows: list[dict[str, str]], summary: list[dict[str, str]]) -> None:
    ...
    rejected_rows = [row for row in rows if row.get("geography_timing_status", "").startswith("rejected_")]
    REPORT_PATH.write_text(
        f"""# ALB_2012 Timing And Geography Exhaustive Audit
...
{markdown_rows(priority_rows, ['source_file', 'variable_name', 'variable_label', 'candidate_role', 'nonmissing_rows', 'household_base_match_rows', 'geography_timing_status'], 50) if priority_rows else 'No priority timing/geography candidates were found.'}

## Rejected Timing/Geography Keyword Rows

{markdown_rows(rejected_rows, ['source_file', 'variable_name', 'variable_label', 'candidate_role', 'nonmissing_rows', 'geography_timing_status'], 50) if rejected_rows else 'No rejected timing/geography keyword rows were found.'}
...
def main() -> None:
    ...
    append_log(TEMP_DIR / "audit_log.md", f"Built ALB_2012 timing/geography exhaustive audit rows={len(rows)} decision={DECISION}.")
    print(f"ALB_2012 timing/geography exhaustive audit rows={len(rows)} decision={DECISION}.")
