...
from pipeline_utils import ROOT, TEMP, ensure_dirs, write_text, append_note
...
def main() -> None:
    ...
    write_text(TEMP / "skill_registry_all.txt", "\n".join(str(p) for p in skill_paths) + "\n")

    matching = []
    needles = [
        "auto-empirical-research", "aer", "empirical", "causal", "econometric",
        "reproduc", "data-audit", "data audit", "econ-write", "economics writing",
        "did", "synthetic-control", "panel-data",
    ]
    for p in skill_paths:
        hay = str(p).lower()
        if any(n in hay for n in needles):
            matching.append(p)
    write_text(TEMP / "skill_registry_matching.txt", "\n".join(str(p) for p in matching) + "\n")

    skill_log = """# Skill Usage Log

Access date: 2026-07-07

The full local registry scan found {n_all} SKILL.md files. Matching empirical, causal,
...
""".format(n_all=len(skill_paths))
    write_text(TEMP / "skill_usage_log.md", skill_log)

    for name in ["audit_log.md", "iteration_notes.md", "rejected_designs.md"]:
        write_text(TEMP / name, f"# {name.replace('_', ' ').replace('.md', '').title()}\n")

    append_note("Phase 0: Skill discovery", [
